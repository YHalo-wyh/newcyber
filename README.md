...
NewCyber 是面向 AI 安全专项、取证流量和恶意样本赛题的本地分析工作台。第一版聚焦赛题接收后的快速分诊：固定证据、识别题型、提取关键线索，并生成可复核、可交接的分析报告。

## 当前能力

- 递归盘点赛题附件，计算 SHA-256、文件类型、大小与字节熵
- 按附件和内容特征判断 AI/ML、取证/流量、恶意样本方向
- 从文本与二进制可打印字符串中提取 Flag、URL、IP 地址候选
- 发现 AI 集成命令拼接、不可信模型反序列化、硬编码凭据等待验证线索
- 识别 WAV、PCAP/PCAPNG、模型文件、压缩包、SQLite、PDF 等格式
- 对 PCM WAV 按 0.1 秒分段，恢复 600Hz 出现/反向比特序列与主频列表
- 汇总 PCAP 中的协议和主要 IPv4 TCP/UDP 会话
- 安全读取 NumPy、SafeTensors、PyTorch ZIP 的结构，不反序列化模型
- 检查压缩包中的路径穿越条目
...
这是通解工具的基础层，不替代 Wireshark、Volatility、模型评测框架等专用工具。后续适配器会在隔离环境中调用这些工具，并把结果回收到同一证据模型中。
