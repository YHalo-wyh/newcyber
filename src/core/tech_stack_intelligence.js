...
function purl(component) {
  ...
  const name = encodeURIComponent(component.name).replace(/%2F/gi,'/').replace(/%40/gi,'@');
  const version = encodeURIComponent(component.version);
  const type = ({ npm:'npm', pypi:'pypi', maven:'maven', golang:'golang', cargo:'cargo', composer:'composer', gem:'gem', docker:'docker' })[component.ecosystem];
  return type ? `pkg:${type}/${name}@${version}` : null;
}
...
function manifestKind(filePath='') {
  ...
  if (base === 'poetry.lock') return 'poetry-lock';
...
function parsePackageLock(text, source) {
  ...
  if (json.packages && typeof json.packages==='object') {
    for (const [pkgPath, meta] of Object.entries(json.packages)) {
      ...
      const name=meta.name || packageNameFromLockPath(pkgPath);
      if (!name) continue;
      out.push(component({ ecosystem:'npm', name, version:String(meta.version), scope:meta.dev?'dev':'resolved', source, direct:false, resolved:true }));
    }
  } else if (json.dependencies && typeof json.dependencies==='object') {
    for (const [name, meta] of Object.entries(json.dependencies)) if (meta?.version) out.push(component({ ecosystem:'npm', name, version:String(meta.version), scope:meta.dev?'dev':'resolved', source, resolved:true }));
  }
  return out.filter(Boolean);
}
...
function parseGradle(text, source) {
  const out=[];
  for (const m of text.matchAll(/\b(?:implementation|api|compileOnly|runtimeOnly|testImplementation|compile)\s*\(?\s*["']([^:"']+):([^:"']+):([^"']+)["']/g)) {
    out.push(component({ ecosystem:'maven', name:`${m[1]}:${m[2]}`, version:exactVersionFromConstraint(m[3])||m[3], constraint:m[3], scope:'gradle', source, direct:true, resolved:true }));
  }
  return out.filter(Boolean);
}
...
function parseGoMod(text, source) {
  const out=[];
  for (const m of text.matchAll(/^\s*([A-Za-z0-9._~/-]+)\s+(v\d+\.\d+(?:\.\d+)?[^\s]*)/gm)) {
    if (m[1]==='module' || m[1]==='go') continue;
    out.push(component({ ecosystem:'golang', name:m[1], version:m[2].replace(/^v/,''), scope:'require', source, direct:true, resolved:true }));
  }
  return out.filter(Boolean);
}
...
function parseManifest(kind, text, source) {
  const fn = ({
    'npm-package':parsePackageJson,
    'npm-lock':parsePackageLock,
    'pip-requirements':parseRequirements,
    'pipfile-lock':parsePipfileLock,
    'poetry-lock':parsePoetryLock,
    'pyproject':parsePyproject,
    'maven-pom':parsePom,
    gradle:parseGradle,
    'go-mod':parseGoMod,
    'cargo-lock':parseCargoLock,
    'cargo-toml':parseCargoToml,
    'composer-json':(value,src)=>parseComposer(value,src,false),
    'composer-lock':(value,src)=>parseComposer(value,src,true),
    'gem-lock':parseGemLock,
...
async function analyzeWorkspaceTechStack(rootPath, analysis) {
  ...
  for (const file of manifestFiles) {
    ...
    try {
      ...
      manifests.push({ path:file.path, kind, components:0, error:error.message });
    }
  }
  ...
  for (const file of bannerCandidates) {
    try {
      ...
      try {
        const size=Math.min(file.size,MAX_BANNER_BYTES);
        const buffer=Buffer.alloc(size);
        const {bytesRead}=await handle.read(buffer,0,size,0);
        const data=buffer.subarray(0,bytesRead);
        if (data.includes(0)) continue;
        components.push(...scanBannerText(data.toString('utf8'),file.path));
        bannerFilesRead += 1;
  ...
  const ecosystems={};
  for (const item of merged) ecosystems[item.ecosystem]=(ecosystems[item.ecosystem]||0)+1;
  const runtimes=merged.filter((item)=>item.ecosystem==='runtime' || item.scope==='base-image' || item.scope==='banner').slice(0,120);
  return {
    ...
    components:merged,
    runtimes,
    purls:[...new Set(purls)].slice(0,800),
    cpeCandidates:cpes.slice(0,160),
    queryText:null
  };
}
...
module.exports={
  MAX_MANIFEST_BYTES,
  manifestKind,
  exactVersionFromConstraint,
  parseManifest,
  scanBannerText,
  mergeComponents,
  queryTextFromTechStack,
  analyzeWorkspaceTechStack
};
