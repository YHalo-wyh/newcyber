...
function addClass(map,name,index,source,sourceFile){if(name===undefined||index===undefined)return;const idx=Number(index);if(!Number.isInteger(idx))return;const key=String(name).trim();if(!key)return;if(!map.has(key))map.set(key,{name:key,index:idx,source,sourceFile});}
function discoverJsonClassMappings(file,body){
  ...
  walk(parsed,(value)=>{
    ...
    for(const key of ['classes','labels','class_names','classNames']){const arr=value[key];if(Array.isArray(arr)&&arr.length>=2&&arr.length<=100000&&arr.every((x)=>typeof x==='string'))arr.forEach((name,index)=>addClass(map,name,index,key,file));}
...
function discoverPythonClassMappings(file,body){
  ...
  const listRe=/(?:classes|labels|class_names)\s*=\s*\[([^\]]{1,20000})\]/gi;while((match=listRe.exec(body))){const names=[];const stringRe=/["']([^"']+)["']/g;let item;while((item=stringRe.exec(match[1])))names.push(item[1]);if(names.length>=2)names.forEach((name,index)=>addClass(map,name,index,'python-class-list',file));}
  return[...map.values()];
}
...
function buildCandidateIndex(sources=[]){
  ...
  const classMap=new Map();for(const row of classes)if(!classMap.has(row.name))classMap.set(row.name,row);
  return{schema:'newcyber.challenge-candidate-index.v1',mappings:mappings.slice(0,MAX_ROWS),classes:[...classMap.values()],stats:{candidateMappings:mappings.length,classMappings:classMap.size,ambiguousBasenames:ambiguousBases.size},_exact:exact,_base:base,_classMap:classMap};
}
...
function normalizeLabel(label,index){const direct=numericMaybe(label);if(index._classMap.has(String(direct)))return index._classMap.get(String(direct)).index;return direct;}
function candidateIdentity(candidatePath,index,hints){
  const relative=norm(candidatePath);let mapped=index._exact.get(relative)||index._base.get(basename(relative))||null;const targets=targetSet(hints);if(mapped){const label=normalizeLabel(mapped.label,index);if(targets.has(String(label)))return{label,id:mapped.id??null,source:mapped.source,sourceFile:mapped.sourceFile,mappedFile:mapped.file};}
  const parts=relative.split('/').slice(0,-1).reverse();for(const part of parts){if(targets.has(String(part)))return{label:numericMaybe(part),id:null,source:'parent-folder',sourceFile:null,mappedFile:null};const cls=index._classMap.get(part);if(cls&&targets.has(String(cls.index)))return{label:cls.index,id:null,source:'class-map+parent-folder',sourceFile:cls.sourceFile,mappedFile:null};}
  return null;
}

module.exports={splitDelimited,discoverJsonCandidateMappings,discoverDelimitedCandidateMappings,discoverJsonClassMappings,discoverPythonClassMappings,buildCandidateIndex,candidateIdentity};
