...
function completeClassCount(index){const values=[...new Set(list(index?.classes).map((row)=>Number(row?.index)).filter(Number.isInteger))].sort((a,b)=>a-b);if(values.length<2)return null;for(let i=0;i<values.length;i+=1)if(values[i]!==i)return null;return values.length;}
...
function normalizeScoreSpace(hints,runs,candidateIndex=null){
  const maps=mapsFromIndex(candidateIndex);const classCount=completeClassCount(candidateIndex);const pairs=[];const unresolved=[];let mappedNames=0;
  ...
  const normalizedRuns=list(runs).map((run,index)=>{const mapped=mapLabel(run?.assignedLabel,maps);if(!mapped.ok)unresolved.push({kind:'assigned-label',index,value:run?.assignedLabel,id:run?.id});if(mapped.source==='class-map')mappedNames+=1;return{...run,assignedLabel:mapped.value,...(classCount?{expectedClasses:classCount}:{})};});
  const mapping={classes:maps.byName.size,mappedNames,completeClassCount:classCount,mode:mappedNames?'class-map-to-output-index':'numeric-index'};
  if(unresolved.length)return{schema:'newcyber.challenge-score-space.v2',status:'gap',hints:pairs,runs:normalizedRuns,unresolved:unresolved.slice(0,64),mapping};
  return{schema:'newcyber.challenge-score-space.v2',status:'ready',hints:pairs,runs:normalizedRuns,unresolved:[],mapping};
}

module.exports={hintPair,numericLabel,mapsFromIndex,completeClassCount,mapLabel,normalizeScoreSpace};
