...
function hintRowsFromSources(sources){
  ...
  for(const item of sources){const json=discoverJsonMaterial(item.file,item.text);for(const row of json.hints)add(row.file,row.rows,row.source);for(const row of discoverTextHints(item.file,item.text))add(row.file,row.rows,row.source);}return sets;
}
...
function chooseHintSet(sets,candidatePaths){
  if(!sets.length)return null;return sets.map((set)=>{let matches=0;for(const file of candidatePaths)if(labelFromPath(file,set.rows)!==null)matches+=1;return{...set,matches};}).sort((a,b)=>b.matches-a.matches||b.rows.length-a.rows.length||a.file.localeCompare(b.file))[0];
}
...
async function runNpyBundle(root,analysis,modelInfo,hintSet,options={}){
  if(modelInfo.model.inputs.length!==1)return{runs:[],errors:[{error:`auto NPY mode requires exactly one ONNX input, got ${modelInfo.model.inputs.length}`}],mode:'npy'};const execute=options.runModel||runOnnxModel;const input=modelInfo.model.inputs[0];const files=npyFiles(analysis).slice(0,MAX_CANDIDATES);const runs=[];const errors=[];
  for(const file of files){const assignedLabel=labelFromPath(file.path,hintSet.rows);if(assignedLabel===null)continue;const target=path.resolve(root,file.path);if(!inside(root,target))continue;try{const stat=await fs.lstat(target);if(!stat.isFile()||stat.isSymbolicLink()||stat.size<=0||stat.size>MAX_NPY_BYTES)throw new Error(`NPY size ${stat.size} outside auto-run limit`);const raw=npyTensorSpec(await fs.readFile(target),file.path);const spec=adaptDimsToModel(raw,input.metadata);const run=await execute(modelInfo.target,{feeds:{[input.name]:{type:spec.type,dims:spec.dims,base64:spec.base64}}},{provider:'cpu'});runs.push({id:idFromPath(file.path),file:file.path,assignedLabel,run,npy:{descr:raw.npy.descr,shape:raw.npy.shape,adaptation:spec.adaptation}});}catch(error){errors.push({file:file.path,error:String(error?.message||error).slice(0,300)});}}
...
async function runImageBundle(root,analysis,modelInfo,hintSet,options={}){
  if(modelInfo.model.inputs.length!==1)return{runs:[],errors:[{error:`auto image mode requires exactly one ONNX input, got ${modelInfo.model.inputs.length}`}],mode:'image'};const manifest=analysis.aiPreprocessingManifest;if(!manifest||manifest.status!=='ready'||manifest.executionReady!==true)return{runs:[],errors:[{error:`preprocessing manifest is ${manifest?.status||'missing'}`}],mode:'image'};const execute=options.runModel||runOnnxModel;const input=modelInfo.model.inputs[0];const files=imageFiles(analysis).slice(0,MAX_CANDIDATES);const runs=[];const errors=[];
  for(const file of files){const assignedLabel=labelFromPath(file.path,hintSet.rows);if(assignedLabel===null)continue;const target=path.resolve(root,file.path);if(!inside(root,target))continue;try{const decoded=await decodeCandidateImage(target,file.path,options);const tensor=preprocessDecodedImage(decoded,manifest,input.metadata);const run=await execute(modelInfo.target,{feeds:{[input.name]:{type:tensor.type,dims:tensor.dims,base64:tensor.base64}}},{provider:'cpu'});runs.push({id:idFromPath(file.path),file:file.path,assignedLabel,run,image:{width:decoded.width,height:decoded.height,tensorDims:tensor.dims,adaptation:tensor.adaptation,backend:tensor.backend,trace:tensor.trace}});}catch(error){errors.push({file:file.path,error:String(error?.message||error).slice(0,360)});}}
  return{runs,errors,mode:'image'};
}

async function runChallengeOnnxAutopilot(rootPath,analysis,options={}){
  const root=path.resolve(String(rootPath||''));const sources=await collectSources(root,analysis);const explicit=discoverExplicitFeedBundle(sources);const npys=npyFiles(analysis);const images=imageFiles(analysis);const hintSets=hintRowsFromSources(sources);const candidatePaths=[...npys,...images].map((x)=>x.path);const hintSet=chooseHintSet(hintSets,candidatePaths);const npyEligible=hintSet?npys.some((file)=>labelFromPath(file.path,hintSet.rows)!==null):false;const imageEligible=hintSet?images.some((file)=>labelFromPath(file.path,hintSet.rows)!==null):false;
  if(!explicit&&!npyEligible&&!imageEligible){const code=!candidatePaths.length?'CANDIDATES_MISSING':!hintSet?'HINTS_MISSING':'CANDIDATE_LABEL_MAPPING_MISSING';return{schema:'newcyber.challenge-onnx-autopilot.v2',runtime:{available:null,source:null,version:null},status:'not-applicable',mode:null,model:null,modelSelection:null,runs:0,errors:[],contest:null,gap:{code,detail:'need explicit feed bundle or label-folder candidates plus hint pairs'}};}
  const mode=explicit?'explicit-feeds':npyEligible?'npy':'image';
  if(mode==='image'&&(analysis.aiPreprocessingManifest?.status!=='ready'||analysis.aiPreprocessingManifest?.executionReady!==true)){const m=analysis.aiPreprocessingManifest;return{schema:'newcyber.challenge-onnx-autopilot.v2',runtime:{available:null,source:null,version:null},status:'gap',mode,model:null,modelSelection:null,runs:0,errors:[],contest:null,gap:{code:'PREPROCESSING_NOT_READY',detail:`image candidates found but preprocessing manifest=${m?.status||'missing'}`,missing:m?.missing||[],conflicts:(m?.conflicts||[]).map((x)=>x.kind)}};}
  const runtime=(options.runtimeStatus||runtimeStatus)();const base={schema:'newcyber.challenge-onnx-autopilot.v2',runtime:{available:runtime.available,source:runtime.source||null,version:runtime.version||null},status:'not-applicable',mode,model:null,modelSelection:null,runs:0,errors:[],contest:null,gap:null};if(!runtime.available)return{...base,status:'gap',gap:{code:'MODEL_RUNTIME_GAP',detail:runtime.installHint||runtime.error||'onnxruntime-node unavailable'}};
  const npyProbe=mode==='npy'?await probeNpyDims(root,npys,hintSet):{dims:null,file:null,errors:[]};const context={mode,explicitBundle:explicit,npyDims:npyProbe.dims,manifest:analysis.aiPreprocessingManifest||null};
  ...
  const selection=modelSelectionView(modelInfo);let executed;try{if(mode==='explicit-feeds')executed=await runExplicitBundle(modelInfo,explicit,options);else if(mode==='npy')executed=await runNpyBundle(root,analysis,modelInfo,hintSet,options);else executed=await runImageBundle(root,analysis,modelInfo,hintSet,options);}catch(error){return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,gap:{code:'INFERENCE_FAILED',detail:String(error?.message||error).slice(0,400)}};}
  const hints=explicit?.hints||hintSet?.rows;if(!executed.runs.length)return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,errors:executed.errors.slice(0,64),gap:{code:'NO_SUCCESSFUL_INFERENCE',detail:'no candidate produced a complete ONNX classification output'}};
  let bridge;try{bridge=rankAdversarialContestFromOnnxRuns({hints,runs:executed.runs,outputName:explicit?.outputName||undefined,shortlistSize:3,beamWidth:2,maxSets:128});}catch(error){return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,runs:executed.runs.length,errors:executed.errors.slice(0,64),gap:{code:'RANKING_FAILED',detail:String(error?.message||error).slice(0,400)}};}
  const synthetic={hints,candidates:bridge.bundle.candidates.map((item)=>({id:item.id,assignedLabel:item.assignedLabel,scores:item.scores}))};const contest=analyzeAiContestBundle([...sources,{file:'__newcyber_onnx_runs__.json',text:JSON.stringify(synthetic)}],analysis,analysis.aiPreprocessingManifest||null);const status=contest.status==='verified'?'verified':'ranked';
  return{...base,status,model:modelInfo.file.path,modelSelection:selection,runs:executed.runs.length,errors:[...(npyProbe.errors||[]),...executed.errors].slice(0,64),bridge:{outputNames:bridge.bridge.outputNames,providers:bridge.bridge.providers,candidateSets:bridge.ranking.candidateSets.slice(0,64)},contest,gap:null};
}

module.exports={dtypeToTensorType,npyTensorSpec,adaptDimsToModel,labelFromPath,idFromPath,chooseHintSet,collectSources,npyFiles,imageFiles,discoverExplicitFeedBundle,probeNpyDims,decodeCandidateImage,runExplicitBundle,runNpyBundle,runImageBundle,runChallengeOnnxAutopilot};
