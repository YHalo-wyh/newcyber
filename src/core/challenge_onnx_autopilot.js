...
function chooseHintSet(sets,candidatePaths,index=null){
  if(!sets.length)return null;return sets.map((set)=>{let matches=0;for(const file of candidatePaths)if(identityFor(file,set.rows,index)!==null)matches+=1;return{...set,matches};}).sort((a,b)=>b.matches-a.matches||b.rows.length-a.rows.length||a.file.localeCompare(b.file))[0];
}
...
async function runNpyBundle(root,analysis,modelInfo,hintSet,options={}){
  if(modelInfo.model.inputs.length!==1)return{runs:[],errors:[{error:`auto NPY mode requires exactly one ONNX input, got ${modelInfo.model.inputs.length}`}],mode:'npy'};const execute=options.runModel||runOnnxModel;const input=modelInfo.model.inputs[0];const files=npyFiles(analysis).slice(0,MAX_CANDIDATES);const runs=[];const errors=[];const candidateIndex=options.candidateIndex||null;
  for(const file of files){const identity=identityFor(file.path,hintSet.rows,candidateIndex);if(!identity)continue;const assignedLabel=identity.label;const target=path.resolve(root,file.path);if(!inside(root,target))continue;try{const stat=await fs.lstat(target);if(!stat.isFile()||stat.isSymbolicLink()||stat.size<=0||stat.size>MAX_NPY_BYTES)throw new Error(`NPY size ${stat.size} outside auto-run limit`);const raw=npyTensorSpec(await fs.readFile(target),file.path);const spec=adaptDimsToModel(raw,input.metadata);const run=await execute(modelInfo.target,{feeds:{[input.name]:{type:spec.type,dims:spec.dims,base64:spec.base64}}},{provider:'cpu'});runs.push({id:identity.id??idFromPath(file.path),file:file.path,assignedLabel,run,labelEvidence:{source:identity.source,sourceFile:identity.sourceFile||null},npy:{descr:raw.npy.descr,shape:raw.npy.shape,adaptation:spec.adaptation}});}catch(error){errors.push({file:file.path,error:String(error?.message||error).slice(0,300)});}}
...
async function runImageBundle(root,analysis,modelInfo,hintSet,options={}){
  if(modelInfo.model.inputs.length!==1)return{runs:[],errors:[{error:`auto image mode requires exactly one ONNX input, got ${modelInfo.model.inputs.length}`}],mode:'image'};const manifest=analysis.aiPreprocessingManifest;if(!manifest||manifest.status!=='ready'||manifest.executionReady!==true)return{runs:[],errors:[{error:`preprocessing manifest is ${manifest?.status||'missing'}`}],mode:'image'};const execute=options.runModel||runOnnxModel;const input=modelInfo.model.inputs[0];const files=imageFiles(analysis).slice(0,MAX_CANDIDATES);const runs=[];const errors=[];const candidateIndex=options.candidateIndex||null;
  for(const file of files){const identity=identityFor(file.path,hintSet.rows,candidateIndex);if(!identity)continue;const assignedLabel=identity.label;const target=path.resolve(root,file.path);if(!inside(root,target))continue;try{const decoded=await decodeCandidateImage(target,file.path,options);const tensor=preprocessDecodedImage(decoded,manifest,input.metadata);const run=await execute(modelInfo.target,{feeds:{[input.name]:{type:tensor.type,dims:tensor.dims,base64:tensor.base64}}},{provider:'cpu'});runs.push({id:identity.id??idFromPath(file.path),file:file.path,assignedLabel,run,labelEvidence:{source:identity.source,sourceFile:identity.sourceFile||null},image:{width:decoded.width,height:decoded.height,tensorDims:tensor.dims,adaptation:tensor.adaptation,backend:tensor.backend,trace:tensor.trace}});}catch(error){errors.push({file:file.path,error:String(error?.message||error).slice(0,360)});}}
  return{runs,errors,mode:'image'};
}

async function runChallengeOnnxAutopilot(rootPath,analysis,options={}){
  const root=path.resolve(String(rootPath||''));const sources=await collectSources(root,analysis);const candidateIndex=buildCandidateIndex(sources);const explicit=discoverExplicitFeedBundle(sources);const npys=npyFiles(analysis);const images=imageFiles(analysis);const hintSets=hintRowsFromSources(sources);const candidatePaths=[...npys,...images].map((x)=>x.path);const hintSet=chooseHintSet(hintSets,candidatePaths,candidateIndex);const npyEligible=hintSet?npys.some((file)=>identityFor(file.path,hintSet.rows,candidateIndex)!==null):false;const imageEligible=hintSet?images.some((file)=>identityFor(file.path,hintSet.rows,candidateIndex)!==null):false;
  if(!explicit&&!npyEligible&&!imageEligible){const code=!candidatePaths.length?'CANDIDATES_MISSING':!hintSet?'HINTS_MISSING':'CANDIDATE_LABEL_MAPPING_MISSING';return{schema:'newcyber.challenge-onnx-autopilot.v3',runtime:{available:null,source:null,version:null},status:'not-applicable',mode:null,model:null,modelSelection:null,candidateIndex:candidateIndex.stats,runs:0,errors:[],contest:null,gap:{code,detail:'need explicit feed bundle or recoverable candidate labels plus hint pairs'}};}
  const mode=explicit?'explicit-feeds':npyEligible?'npy':'image';
  if(mode==='image'&&(analysis.aiPreprocessingManifest?.status!=='ready'||analysis.aiPreprocessingManifest?.executionReady!==true)){const m=analysis.aiPreprocessingManifest;return{schema:'newcyber.challenge-onnx-autopilot.v3',runtime:{available:null,source:null,version:null},status:'gap',mode,model:null,modelSelection:null,candidateIndex:candidateIndex.stats,runs:0,errors:[],contest:null,gap:{code:'PREPROCESSING_NOT_READY',detail:`image candidates found but preprocessing manifest=${m?.status||'missing'}`,missing:m?.missing||[],conflicts:(m?.conflicts||[]).map((x)=>x.kind)}};}
  const runtime=(options.runtimeStatus||runtimeStatus)();const base={schema:'newcyber.challenge-onnx-autopilot.v3',runtime:{available:runtime.available,source:runtime.source||null,version:runtime.version||null},status:'not-applicable',mode,model:null,modelSelection:null,candidateIndex:candidateIndex.stats,runs:0,errors:[],contest:null,gap:null};if(!runtime.available)return{...base,status:'gap',gap:{code:'MODEL_RUNTIME_GAP',detail:runtime.installHint||runtime.error||'onnxruntime-node unavailable'}};
  const npyProbe=mode==='npy'?await probeNpyDims(root,npys,hintSet,candidateIndex):{dims:null,file:null,errors:[]};const context={mode,explicitBundle:explicit,npyDims:npyProbe.dims,manifest:analysis.aiPreprocessingManifest||null};
...
module.exports={dtypeToTensorType,npyTensorSpec,adaptDimsToModel,labelFromPath,idFromPath,identityFor,chooseHintSet,collectSources,npyFiles,imageFiles,discoverExplicitFeedBundle,probeNpyDims,decodeCandidateImage,runExplicitBundle,runNpyBundle,runImageBundle,runChallengeOnnxAutopilot};
