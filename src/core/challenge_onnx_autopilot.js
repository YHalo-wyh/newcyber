...
const {runtimeStatus,runOnnxModel,withSession,runWithOpenSession}=require('./local_ml_runtime');
...
async function runWithInferenceSession(modelInfo,runner,options={}){
  if(typeof options.runModel==='function'){
    const result=await runner({...options,runModel:options.runModel});
    return{...result,inference:{strategy:'injected-run-model',sessionCreates:null,attempted:(result.runs?.length||0)+(result.errors?.length||0),succeeded:result.runs?.length||0,failed:result.errors?.length||0}};
  }
  const openSession=options.withSession||withSession;const runOpen=options.runWithOpenSession||runWithOpenSession;let attempts=0;
  const result=await openSession(modelInfo.target,{...options,provider:'cpu'},async(session,ort)=>{
    const execute=async(_model,request,executionOptions={})=>{attempts+=1;return runOpen(session,ort,request,{provider:'cpu',...executionOptions});};
    return runner({...options,runModel:execute});
  });
  return{...result,inference:{strategy:'single-session-stream',sessionCreates:1,attempted:attempts,succeeded:result.runs?.length||0,failed:result.errors?.length||0}};
}
...
async function runChallengeOnnxAutopilot(rootPath,analysis,options={}){
  ...
  const selection=modelSelectionView(modelInfo),planView=inputPlanView(modelInfo.inputPlan);const runOptions={...options,candidateIndex};let executed;try{const runner=async(executionOptions)=>{if(mode==='explicit-feeds')return runExplicitBundle(modelInfo,explicit,executionOptions);if(mode==='npy')return runNpyBundle(root,analysis,modelInfo,hintSet,executionOptions);return runImageBundle(root,analysis,modelInfo,hintSet,executionOptions);};executed=await runWithInferenceSession(modelInfo,runner,runOptions);}catch(error){return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,inputPlan:planView,gap:{code:'INFERENCE_FAILED',detail:String(error?.message||error).slice(0,400)}};}
  const rawHints=explicit?.hints||hintSet?.rows;if(!executed.runs.length)return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,inputPlan:planView,inference:executed.inference,errors:executed.errors.slice(0,64),gap:{code:'NO_SUCCESSFUL_INFERENCE',detail:'no candidate produced a complete ONNX classification output'}};
  ...
  if(scoreSpace.status!=='ready')return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,inputPlan:planView,scoreSpace:scoreSpaceView,inference:executed.inference,runs:executed.runs.length,errors:[...(npyProbe.errors||[]),...executed.errors].slice(0,64),gap:{code:'LABEL_SPACE_UNRESOLVED',detail:'hint/assigned labels cannot be mapped to ONNX score indices',unresolved:scoreSpace.unresolved}};
  let bridge;try{bridge=rankAdversarialContestFromOnnxRuns({hints:scoreSpace.hints,runs:scoreSpace.runs,outputName:explicit?.outputName||undefined,shortlistSize:3,beamWidth:3,maxSets:256});}catch(error){return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,inputPlan:planView,scoreSpace:scoreSpaceView,inference:executed.inference,runs:executed.runs.length,errors:executed.errors.slice(0,64),gap:{code:'RANKING_FAILED',detail:String(error?.message||error).slice(0,400)}};}
  if(!bridge.ranking.candidateSets.length)return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,inputPlan:planView,scoreSpace:scoreSpaceView,inference:executed.inference,runs:executed.runs.length,errors:[...(npyProbe.errors||[]),...executed.errors].slice(0,64),gap:{code:'NO_RANKED_CANDIDATE_SET',detail:'model inference succeeded but no hint group produced a complete candidate set',findings:bridge.ranking.findings}};
  ...
  return{...base,status,model:modelInfo.file.path,modelSelection:selection,inputPlan:planView,scoreSpace:scoreSpaceView,verifier:{status:verifier.status,specs:verifier.specs.slice(0,16),matches:verifier.matches.slice(0,16)},inference:executed.inference,runs:executed.runs.length,errors:[...(npyProbe.errors||[]),...executed.errors].slice(0,64),bridge:{outputNames:bridge.bridge.outputNames,providers:bridge.bridge.providers,candidateSets:bridge.ranking.candidateSets.slice(0,128)},contest,gap:null};
}

module.exports={dtypeToTensorType,npyTensorSpec,adaptDimsToModel,labelToken,safeHintPair,discoverNamedJsonHints,hintRowsFromSources,labelFromPath,idFromPath,identityFor,chooseHintSet,collectSources,npyFiles,imageFiles,discoverExplicitFeedBundle,probeNpyDims,chosenPrimaryInput,mergeAutomaticFeeds,runWithInferenceSession,decodeCandidateImage,runExplicitBundle,runNpyBundle,runImageBundle,mergeAdvancedVerification,runChallengeOnnxAutopilot};
