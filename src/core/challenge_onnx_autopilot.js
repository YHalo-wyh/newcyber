...
async function runChallengeOnnxAutopilot(rootPath,analysis,options={}){
  ...
  if(!explicit&&!npyEligible&&!imageEligible){const code=!candidatePaths.length?'CANDIDATES_MISSING':!hintSet?'HINTS_MISSING':'CANDIDATE_LABEL_MAPPING_MISSING';return{schema:'newcyber.challenge-onnx-autopilot.v5',runtime:{available:null,source:null,version:null},status:'not-applicable',mode:null,model:null,modelSelection:null,candidateIndex:candidateIndex.stats,scoreSpace:null,verifier:null,runs:0,errors:[],contest:null,gap:{code,detail:'need explicit feed bundle or recoverable candidate labels plus hint pairs'}};}
  const mode=explicit?'explicit-feeds':npyEligible?'npy':'image';
  if(mode==='image'&&(analysis.aiPreprocessingManifest?.status!=='ready'||analysis.aiPreprocessingManifest?.executionReady!==true)){const m=analysis.aiPreprocessingManifest;return{schema:'newcyber.challenge-onnx-autopilot.v5',runtime:{available:null,source:null,version:null},status:'gap',mode,model:null,modelSelection:null,candidateIndex:candidateIndex.stats,scoreSpace:null,verifier:null,runs:0,errors:[],contest:null,gap:{code:'PREPROCESSING_NOT_READY',detail:`image candidates found but preprocessing manifest=${m?.status||'missing'}`,missing:m?.missing||[],conflicts:(m?.conflicts||[]).map((x)=>x.kind)}};}
  const runtime=(options.runtimeStatus||runtimeStatus)();const base={schema:'newcyber.challenge-onnx-autopilot.v5',runtime:{available:runtime.available,source:runtime.source||null,version:runtime.version||null},status:'not-applicable',mode,model:null,modelSelection:null,candidateIndex:candidateIndex.stats,scoreSpace:null,verifier:null,runs:0,errors:[],contest:null,gap:null};if(!runtime.available)return{...base,status:'gap',gap:{code:'MODEL_RUNTIME_GAP',detail:runtime.installHint||runtime.error||'onnxruntime-node unavailable'}};
  ...
  const rawHints=explicit?.hints||hintSet?.rows;if(!executed.runs.length)return{...base,status:'gap',model:modelInfo.file.path,modelSelection:selection,errors:executed.errors.slice(0,64),gap:{code:'NO_SUCCESSFUL_INFERENCE',detail:'no candidate produced a complete ONNX classification output'}};
