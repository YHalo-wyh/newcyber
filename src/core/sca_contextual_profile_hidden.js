...
function parseStaticIntegerList(body){
  ...
  if(!values.length||values.length>MAX_PROFILE_PROMPTS||values.some((value)=>!Number.isSafeInteger(value)||value<=0||value>MAX_TOKEN_SEQUENCE))return null;
  return values;
}
function staticProfilingRowCounts(sourceText,totalTokens=null){
  ...
  const selected=[...canonical.values()][0];const total=selected.counts.reduce((sum,value)=>sum+value,0);
  if(totalTokens!=null&&Number(totalTokens)!==total)return {status:'gap',code:'PROFILE_BOUNDARY_COUNT_GAP',detail:`${selected.name} sum=${total} != profiling token count=${Number(totalTokens)}`,source:selected.name,counts:selected.counts,total};
  return {status:'ok',source:selected.name,counts:selected.counts,total,prompts:selected.counts.length,evidence:'static-source-integer-list'};
}
...
module.exports={MAX_PROFILE_PROMPTS,MAX_PROFILE_TOKENS,ROW_COUNT_NAMES,parseStaticIntegerList,staticProfilingRowCounts,reconstructProfilingSequences,extractSequenceHidden,captureContextualProfileHiddenStates};
