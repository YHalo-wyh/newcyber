...
function directionClosure(direction,analysis,context){
  ...
  if(id==='privacy-leakage'){
    ...
    const verifiedObs=has(/ai-transform-replay-verify[^]{0,300}(?:verified|flag)|observation-verified/i,context);
    const candidate=has(/transform-exfiltration|prompt[^]{0,100}candidate|candidateObject[^]{0,200}(?:prompt|transform)/i,context);
    const waiting=has(/ai-transform-replay-verify|observationHandoff/i,context);
    if(verifiedObs){steps=1;stage='verified-observation-needs-flag';readiness='verifier-ready';next='从已验证响应执行 ASCII/hex/base64/URL/Unicode 解码并提取最终 flag。';}
    else if(candidate&&waiting){steps=1;stage='candidate-awaiting-observation';readiness='remote-replay-ready';blockers.push('REAL_TARGET_OBSERVATION');next='使用推荐 Prompt replay contract 获取真实响应，回填 observation 后自动 verifier。';}
    ...
    const candidate=has(/adversarial[^]{0,160}candidate|candidate[^]{0,160}adversarial|epsilon[^]{0,100}(?:valid|within|pass)/i,context);
    ...
    const trigger=has(/trigger[^]{0,140}candidate|patch[^]{0,140}candidate|backdoor[^]{0,140}candidate/i,context);
    ...
    const sideEffect=has(/torchscript[^]{0,180}side.effect|file.side.effect|pickle[^]{0,120}(?:global|reduce)|unsafe[^]{0,100}load/i,context);
    const obs=has(/torchscript-file-side-effect-chain[^]{0,260}(?:verified|captured)|observation-verified/i,context);
    if(obs){steps=1;stage='side-effect-observed';readiness='verifier-ready';next='把已验证 side effect 与题目 checker/flag 路径绑定，提取结果而不是继续做泛化 SCA。';}
...
function buildFlagClosureScheduler(analysis={},autopilot=analysis.aiCompetitionAutopilot||{}){
  ...
  return {
    ...
    policy:{optimizeFor:'flag-closure-distance',preserveAllFiveDirections:true,remoteExecution:'authorization-required'}
  };
}

module.exports={globalVerified,directionClosure,buildFlagClosureScheduler};
