...
async function enrichFirmware(rootPath, file) {
  ...
  file.findings ||= [];
  for (const finding of summary.findings || []) file.findings.push({ ...finding, file: file.path, count: 1, id: `${finding.id}:${file.path}` });
...
async function enrichUavText(rootPath, file) {
  ...
  file.metadata = {
    ...
    uavChallenge: {
      ...
      hits: result.hits.slice(0, 12),
      telemetry: result.telemetry ? { summary: result.telemetry.summary, anomalies: result.telemetry.anomalies.slice(0, 20) } : null,
      control: result.controlFlow ? {
        streams: result.controlFlow.streams,
        paramWrites: result.controlFlow.paramWrites.slice(0, 30),
        modeChanges: result.controlFlow.modeChanges.slice(0, 30),
        commands: result.controlFlow.commands.slice(0, 30).map((x) => ({ command: x.command, ack: x.ack, accepted: x.accepted })),
        findings: result.controlFlow.findings.slice(0, 20)
      } : null
    }
  };
  file.findings ||= [];
...
function buildUavSection(analysis) {
  ...
  for (const file of analysis.files || []) {
    ...
    if (uav?.hits?.length) {
      ...
      if (uav.telemetry?.anomalies?.length) lines.push(`- 遥测一致性异常：${uav.telemetry.anomalies.slice(0, 6).map((x) => `${x.id}@frame${x.frameIndex}`).join(', ')}`);
      if (uav.control?.findings?.length) lines.push(`- 控制状态机：${uav.control.findings.slice(0, 6).map((x) => x.id).join(', ')}`);
      lines.push('');
    }
  }
