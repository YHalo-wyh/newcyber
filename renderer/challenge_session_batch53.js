(() => {
  ...
  function stageState(kind,value){
    ...
    if(kind==='verifier')return value?.status==='verified'?'done':value?.status==='ranked'?'partial':value?.status==='partial'?'partial':'idle';
    return'idle';
  }
