(() => {
  ...
  const CASES=[
    ...
    ['用户提供训练题','06-ai_summarizer-攻击','PROMPT → ENCODED EXFIL','PARTIAL'],
    ['用户提供训练题','07-ai_sms-攻击','TORCHSCRIPT → FILE SIDE EFFECT','PARTIAL']
  ];
  ...
  function emptyResult(){
    return `<div class="real-ctf-empty"><span>AI / REAL CORPUS</span><strong>${CASES.length} 个公开/用户提供真题样本待回归</strong><p>结果分三层：Recognized 只代表识别核心证据；Candidate 必须产出具体候选；Verified 还要完成 oracle / 确定性验证。不会把 WP 里的现成 flag 或成功截图计为 Verified。</p></div>`;
  }
  ...
  function toolPage(){
    ...
    return `<div class="page-head tool-head real-ctf-head"><div><span class="kicker">AI · REAL CHALLENGE MATURITY</span><h1>国内 AI CTF 真题回归</h1><p>公开题面 / 官方源码 / 用户提供题包 / 公开题解 → 最小复现 → Recognized → Candidate → Verified。Candidate 会继续接 verifier；识别到漏洞模式不会包装成“已经做出题”。</p></div><button class="button ghost" data-view="ai">返回</button></div>
      <div class="real-ctf-workbench">
        <article class="panel real-ctf-catalog"><div class="result-title"><div><b>真题集</b><small>${CASES.length} CASES · 国内赛事优先</small></div></div><div class="real-ctf-list">${catalogRows()}</div><div class="real-ctf-source-note">来源：USTC Hackergame · SUCTF 官方仓库 · CTF-Archives · 公开赛事题解 · 用户提供训练题包。未公开附件细节不会被补写进 fixture。</div></article>
